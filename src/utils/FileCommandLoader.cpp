...
bool FileCommandLoader::LoadFile(const std::string &filePath)
{
  ...
  while (!inFile.eof())
  {
    ...
    {
      size_t dlimitPos = line.find_first_of(" ", commandPos);
      if (dlimitPos == std::string::npos)
      {
        dlimitPos = line.length();
      }
      else
      {
        dlimitPos -= 1;
      ...
      std::string commandStr = line.substr(commandPos + 1, dlimitPos);
      dlimitPos += 1;
      ...
      for (size_t commandIndex = 0; commandIndex < mCommands.size(); ++commandIndex)
      {
        if (commandStr == mCommands[commandIndex].command)
        {
          ...
          {
            std::string numLines = line.substr(dlimitPos + 1);
